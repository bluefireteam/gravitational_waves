...
class NineBox {
  ...
  void draw(Canvas c, double x, double y, double width, double height) {
    ...
    _drawTile(
        c, _getDest(x + tileSize, y + height - tileSize, width: mx), 1, 2);

    // vertical sides
    double my = height - 2 * tileSize;
    _drawTile(c, _getDest(x, y + tileSize, height: my), 0, 1);
    _drawTile(
        c, _getDest(x + width - tileSize, y + tileSize, height: my), 2, 1);

    // center
    _drawTile(
        c, _getDest(x + tileSize, y + tileSize, width: mx, height: my), 1, 1);
  }
